...
{'Back-End / Server-Side'|helper:'page':'headline'}
<div class="row">
    <div class="col-lg-12 col-md-12">
        {$page->headline}
        <p>Typically, a server is a computer program, such as a {'web server'|link_urls:'w'}, that runs on a remote server, reachable from a user's local computer or workstation.</p>
        <p>Operations may be performed server-side because they require access to information or functionality that is not available on the client, or require typical behaviour that is unreliable when it is done client-side.</p>
        ...
        <p>Examples of server-side processing include the creation and adaptation of a database using {'MySQL'|link_urls:'w'}.</p>
        {''|helper:'page':'useMore'}
    </div>
</div>
{if $page->sub == FALSE}
<div class="row">
    <div class="col-lg-12 col-md-12">
        ...
        <p><strong>Server-side</strong> scripting is a technique used in website design which involves embedding scripts in an {'HTML'|link_urls:'w'} source code which results in a user's (client's) request to the server website being handled by a script running on the server-side before the server responds to the client's request.</p>
        ...
        <p>When the server serves data in a commonly used manner, for example according to the {'HTTP'|link_urls:'w'} or {'FTP'|link_urls:'w'} protocols, users may have their choice of a number of client programs (most modern web browsers can request and receive data using both of those protocols).</p>
        ...
        <p>{'Netscape'|link_urls:'/c'} introduced an implementation of <strong>JavaScript for server-side ({'JSS'|link_urls:'w'})</strong> scripting with <strong>Netscape Enterprise Server</strong>, first released in December, 1994 (soon after releasing {'JavaScript'|link_urls:'/l'} for browsers).</p>
        ...
        <p>A more commonly used service is Google's proprietary search engine, which searches millions of cached results related to the user specified keyword and returns an ordered list of links back to the client.</p>
        ...
        <p>In the earlier days of the web, server-side scripting was almost exclusively performed by using a combination of {'C'|link_urls:'/l'} programs, {'Perl'|link_urls:'/l'} scripts, and {'shell scripts'|link_urls:'i'} using the <strong>Common Gateway Interface ({'CGI'|link_urls:'w'})</strong>.</p>
        <p>Those scripts were executed by the operating system, and the results were served back by the web server. Many modern web servers can directly execute on-line scripting languages such as {'ASP'|link_urls:'/l'} and {'PHP'|link_urls:'/l'} either by the web server itself or via extension modules (e.g. <code>mod_perl</code> or <code>mod_php</code>) to the web server.</p>
        ...
        <p>Dynamic websites sometimes use custom web application servers, such as the {'Python'|link_urls:'/l'} "Base HTTP Server" library, although some may not consider this to be server-side scripting.</p>
