...
{'NewSQL'|helper:'page':'headline'}
<div class="row">
    <div class="col-lg-12 col-md-12">
        {$page->headline}
        <p><strong>NewSQL</strong> is a class of modern relational database management systems that seek to provide the same scalable performance of {'NoSQL'|link_urls:'w'} systems for on-line transaction processing (read-write) workloads while still maintaining the {'ACID'|link_urls:'i'} guarantees of a traditional database system.</p>
        ...
        <p>The only options previously available for these organizations were to either purchase a more powerful single-node machine or develop custom middleware that distributes queries over traditional {'DBMS'|link_urls:'w'} nodes.</p>
        <p>Both approaches are prohibitively expensive and thus are not an option for many.</p>
        <p>Thus, in this paper, Aslett discusses how NewSQL upstarts are poised to challenge the supremacy of commercial vendors, in particular {'Oracle'|link_urls:'/c'}.</p>
        <p>Although NewSQL systems vary greatly in their internal architectures, the two distinguishing features common amongst them is that they all support the relational data model and use {'SQL'|link_urls:'w'} as their primary interface.</p>
